...
area2_tab_panels = {}
active_area1_tab = 'tab1_1'  # Default active tab in Area 1
active_area2_tab = 'tab2_1'  # Default active tab in Area 2
...
def on_area1_tab_change(e):
    """Update active tab when Area 1 tab is clicked"""
    global active_area1_tab
    active_area1_tab = e.value

def on_area2_tab_change(e):
    """Update active tab when Area 2 tab is clicked"""
    global active_area2_tab
    active_area2_tab = e.value
...
def create_content_areas():
    ...
    global area1_tabs, area2_tabs, area1_tab_panels, area2_tab_panels, active_area1_tab, active_area2_tab
    ...
    with splitter.before:
        ...
        with area1_wrapper:
            with ui.tabs().classes('w-full').on('update:model-value', on_area1_tab_change) as area1_tabs:
            ...
            with ui.tab_panels(area1_tabs, value=active_area1_tab).classes('w-full h-full'):
    ...
    with splitter.after:
        ...
        with area2_wrapper:
            with ui.tabs().classes('w-full').on('update:model-value', on_area2_tab_change) as area2_tabs:
            ...
            with ui.tab_panels(area2_tabs, value=active_area2_tab).classes('w-full h-full'):
...
def load_example_page_1():
    ...
    global active_area1_tab, area1_tab_panels, area1_tabs
    
    # Get the active tab's scroll area
    active_panel = area1_tab_panels[active_area1_tab]
    
    # Clear and load new content
    active_panel.clear()
    with active_panel:
        ui.label('Example Page 1').classes('text-3xl font-bold mb-4 text-blue-600')
        ui.label(f'Loaded in {active_area1_tab.upper()}').classes('text-lg mb-2')
        
        for i in range(1, 21):
            ui.label(f'Line {i}: Sample text content for Area 1').classes('mb-2')
    
    # Update tab label to reflect new content
    for tab in area1_tabs:
        if hasattr(tab, 'name') and tab.name == active_area1_tab:
            tab.props('label="Page 1"')
            break
    
    ui.notify(f'Loaded Example Page 1 in Area 1 - {active_area1_tab}')

def load_example_page_2():
    """Load example content in the active tab of Area 2"""
    global active_area2_tab, area2_tab_panels, area2_tabs
    
    # Get the active tab's scroll area
    active_panel = area2_tab_panels[active_area2_tab]
    
    # Clear and load new content
    active_panel.clear()
    with active_panel:
        ui.label('Example Page 2').classes('text-3xl font-bold mb-4 text-green-600')
        ui.label(f'Loaded in {active_area2_tab.upper()}').classes('text-lg mb-2')
        
        for i in range(1, 21):
            ui.label(f'Line {i}: Sample text content for Area 2').classes('mb-2')
    
    # Update tab label to reflect new content
    for tab in area2_tabs:
        if hasattr(tab, 'name') and tab.name == active_area2_tab:
            tab.props('label="Page 2"')
            break
    
    ui.notify(f'Loaded Example Page 2 in Area 2 - {active_area2_tab}')
